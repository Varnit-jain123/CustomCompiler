  ...
  async getTemplate(req, res) {
    try {
      ...
      const templates = {
        blink: {
          avr: `void setup() {\n  pinMode(LED_BUILTIN, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(LED_BUILTIN, HIGH);\n  delay(1000);\n  digitalWrite(LED_BUILTIN, LOW);\n  delay(1000);\n}`,
          esp32: `#define LED_PIN 2\n\nvoid setup() {\n  pinMode(LED_PIN, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(LED_PIN, HIGH);\n  delay(1000);\n  digitalWrite(LED_PIN, LOW);\n  delay(1000);\n}`,
          stm32: `#define LED_PIN PC13\n\nvoid setup() {\n  pinMode(LED_PIN, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(LED_PIN, HIGH);\n  delay(1000);\n  digitalWrite(LED_PIN, LOW);\n  delay(1000);\n}`
        },
        serial: {
          common: `void setup() {\n  Serial.begin(9600);\n  Serial.println("Serial started!");\n}\n\nvoid loop() {\n  if (Serial.available()) {\n    String data = Serial.readStringUntil('\\n');\n    Serial.print("Received: ");\n    Serial.println(data);\n  }\n  delay(100);\n}`
        },
        analog_read: {
          common: `#define ANALOG_PIN A0\n\nvoid setup() {\n  Serial.begin(9600);\n}\n\nvoid loop() {\n  int value = analogRead(ANALOG_PIN);\n  Serial.print("Value: ");\n  Serial.println(value);\n  delay(500);\n}`
        },
        pwm: {
          common: `#define PWM_PIN 9\n\nvoid setup() {\n  pinMode(PWM_PIN, OUTPUT);\n}\n\nvoid loop() {\n  for (int i = 0; i <= 255; i++) {\n    analogWrite(PWM_PIN, i);\n    delay(10);\n  }\n  for (int i = 255; i >= 0; i--) {\n    analogWrite(PWM_PIN, i);\n    delay(10);\n  }\n}`
        },
        wifi_scan: {
          esp32: `#include <WiFi.h>\n\nvoid setup() {\n  Serial.begin(115200);\n  WiFi.mode(WIFI_STA);\n  WiFi.disconnect();\n}\n\nvoid loop() {\n  Serial.println("Scanning...");\n  int n = WiFi.scanNetworks();\n  Serial.print(n);\n  Serial.println(" networks found");\n  for (int i = 0; i < n; i++) {\n    Serial.print(i + 1);\n    Serial.print(": ");\n    Serial.println(WiFi.SSID(i));\n  }\n  delay(5000);\n}`
        },
        interrupt: {
          common: `#define BUTTON_PIN 2\n#define LED_PIN 13\nvolatile bool state = LOW;\n\nvoid setup() {\n  pinMode(LED_PIN, OUTPUT);\n  pinMode(BUTTON_PIN, INPUT_PULLUP);\n  attachInterrupt(digitalPinToInterrupt(BUTTON_PIN), toggle, FALLING);\n}\n\nvoid loop() {\n  digitalWrite(LED_PIN, state);\n}\n\nvoid toggle() {\n  state = !state;\n}`
        }
      };
      ...
      if (boardId) {
        const board = boardsConfig.getBoard(boardId);
        if (board) {
          code = templates[templateId][board.architecture] || templates[templateId].common;
        } else {
          code = templates[templateId].common;
        }
      } else {
