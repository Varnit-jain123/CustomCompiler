...
//       // Determine variant directory based on board
//       let variantName = 'standard'; // Default for Uno
//       if (boardConfig.id === 'arduino_mega') {
//         variantName = 'mega';
//       }
...
//       logger.info(`Building core library for ${boardConfig.name}`);
//       logger.info(`  Core dir: ${coreDir}`);
//       logger.info(`  Variant dir: ${variantDir}`);

//       // Include paths for core compilation
//       const includePaths = [coreDir, variantDir];
...
//           const objectFile = path.join(coreOutputDir, `${file}.o`);
          
//           logger.info(`  Compiling C file: ${file}`);
...
//       logger.info(`Creating core archive with ${objectFiles.length} object files`);
//       await this.executeCommand(arCommand, arArgs);
      
//       logger.info('Built Arduino core library successfully');
